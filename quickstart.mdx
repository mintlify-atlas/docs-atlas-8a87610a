...
<Steps>
  ...
  <Step title="Preview the production build">
    ...
    Starts a local static file server at [http://localhost:4173](http://localhost:4173) serving the `public/` output directory, with the same `0.0.0.0` binding as the dev server.
  </Step>
